...
/// Check if the input has balanced delimiters (parentheses, brackets, braces)
fn is_balanced(input: &str) -> bool {
    ...
    // Expression is balanced if all counts are zero and we're not in a string
...
fn main() {
    ...
    loop {
        ...
        match rustyline_readline(prompt) {
            Some(line) => {
                ...
                // If not balanced, continue accumulating
            }
            None => {
                println!();
                break;

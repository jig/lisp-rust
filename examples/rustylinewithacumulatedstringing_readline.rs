        ...
        match rustyline_readline(prompt) {
            Some(line) => {
                ...
                match rep(&accumulated, &env) {
                    ...
                    Err(e) => {
                        // Check if it's an incomplete expression error
                        if let mal::types::MalVal::Str(ref err_msg) = e {
                            if err_msg.starts_with("INCOMPLETE:") {
                                // Continue reading more input
                                continue;
                            }
                        }
                        // Real error, show it and reset
                        println!("Error: {}", e.pr_str(true));
                        accumulated.clear();
                    }
                }
            }
